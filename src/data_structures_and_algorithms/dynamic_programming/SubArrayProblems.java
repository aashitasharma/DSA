...
public class SubArrayProblems {
	...
	public static int count(int[] a) {
		...
		for (int i = 0; i < a.length - 1; i++) {
			if (a[i] < a[i + 1]) {
				...
				//adding total subarrays with length l
				c += l * (l - 1) / 2;
				l = 1;
			}
		}
	 ...
	 * O(n) solution 
	 * for O(nlogn) use binary search over cumulative sum array
	 */
	public int minSubArrayLen(int[] a, int s){
		...
		int sum=0;
		while(end<a.length){
			while(end<a.length && sum<s) sum+=a[end++];
			if(sum<s)
				break;
			while(start<end && sum>=s) sum-=a[start++];
