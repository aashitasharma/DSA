...
3) check if the linked list is a palindrome without using any extra space and with the lowest possible complexity(both time and space).

4) write code for following API
...
Amazon:
	...
	Finding the Nth element of a binary search tree; 
	Selecting an optimal sort algorithm for a given type of data - ended up coding radix sort, 	basically; 
	there was one about validating an expression with parentheses; 
	Function for flood-filling a 2D matrix; 
...
Microsoft:
	...
	counting of palindromes within a string, 
	identifying the possibility to sort a partially sorted linked list with a single swap, 
	Hanoi towers, 
	finding Top K elements of an array 	
...
Microsoft (India IDC, Full Time):
    ...
    1. Given 2 sorted arrays on size n, find the median of both of them are merged.

Amazon (Internship, India):

    1. Deleting element from Linked List. Concurrency issues that might occur. Possible solutions.
    2. Finding diameter of a tree. The question was a story. Boiled down to this.
    3. Some basic number recurrence and DP. Like Fibonacci.
